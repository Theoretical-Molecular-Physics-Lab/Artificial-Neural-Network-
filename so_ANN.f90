...
implicit none
!
type(C_PTR) :: ann_SO,ann_dv
real,parameter:: dx = 1.0e-5 ! step of the derivative
real,parameter:: dr_inf = 0.826489687 !Parameter inserted for first derivative of  V(R)
...
!---------------------------------------------------------------------
	...
	!
       ...
       !                                       network for V'(R)
       !
	ann_dV = fann_create_from_file(f_c_string('dV_SO.dat')) ! 
	!
	return
	...
	!
       ...
       !                                       network for V(R)
       !
	ann_SO = fann_create_from_file(f_c_string('NN_SO.dat')) ! 
	!
	return
...
!----------------------------------------------------------------------------------------------------------------
    ...
    !
    r(1) = x
    v=f_fann_run(ann_SO,r)
    pot=v(1)  - 0.196117133 !! well depth value for O2 molecule (in a.u.)
    return
    end function pot
!----------------------------------------------------------------------
!----------------------------------------------------------------------
...
!----------------------------------------------------------------------
    ...
    !   
    der_1 = 0.5*(pot(x+dx) - pot(x-dx))/dx  + 1.0 ! To avoid a division by zero, a non-zero value must be inserted
    ! 
    return
    end function der_1
   !-----------------------------------------------------------------------------------
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    der_2=vv(1) - 1.06781042 !1.89426863 ! 
    return
    end function der_2
    !----------------------------------------------------------------------------------
...
open(UNIT=mrm_a+1,FILE='so_potential.res')       
! 
call initialize()
call begin()
write(*,*)' '                                                                                         
write(*,*)' '                                                                                         
write(*,*)'Diatomic system: SO (ground state)' 
write(*,*)'Level of theory: MRCI/AV(5+d)Z' 
...
write(*,*)'for SO molecule' 
write(*,*)' '                                                                                         
write(*,*)' '                                                                                         
write(*,*)' '                                                                                         
write(*,*)'Range of distances used in training: 1.8 to 22.0'
write(*,*)'------------------------------------------------------------------------------------- '
write(*,*)'------------------------------------------------------------------------------------- '
